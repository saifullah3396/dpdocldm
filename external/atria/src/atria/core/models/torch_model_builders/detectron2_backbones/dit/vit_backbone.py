...
from transformers import AutoConfig, LayoutLMv3Model
    ...
    def __init__(
        ...
        if "beit" in name or "dit" in name:
            ...
            self.backbone = LayoutLMv3Model(
                config, detection=True, out_features=out_features, image_only=False
            )
        else:
            self.backbone = model_func(
                img_size=img_size,
...
def build_VIT_backbone(cfg):
    ...
    model_config = eval(str(cfg.MODEL.VIT.MODEL_KWARGS).replace("`", ""))
    ...
    return VIT_Backbone(name, out_features, drop_path, img_size, pos_type, model_config,
