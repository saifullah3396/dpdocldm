...
def from_dict(
    data_class: Type[T], data: Data, config: Optional[Config] = Config(strict=True)
) -> T:
    ...
    if "_target_" in data:
        data_class_at_runtime = data.pop("_target_")
        data_class = getattr(
            import_module(".".join(data_class_at_runtime.split(".")[:-1])),
            data_class_at_runtime.split(".")[-1],
        )
        config.type_hooks[data_class_at_runtime] = data_class
    try:
