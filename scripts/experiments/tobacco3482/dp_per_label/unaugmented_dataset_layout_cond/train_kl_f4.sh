...
# first run with dp_latent_diffusion/with_feature_extraction and then with dp_latent_diffusion/preextracted_features
for LABEL in {0..9}; do
    PYTHONPATH=$PYTHONPATH TASK=document_classification DATASET=tobacco3482_with_ocr TASK_MODULE=klf4_layout_cond \
        ...
        image_size=256 \
        experiment_name=dp_unaug_layout_cond_klf4_cfg_per_label \
