...
# PYTHONPATH=$PYTHONPATH TASK=document_classification DATASET=iitcdip TASK_MODULE=klf4_cfg python $BASE_DIR/external/atria/src/atria/core/task_runners/atria_trainer.py \
#     hydra.searchpath=[pkg://atria/conf,pkg://docsets/conf,pkg://dp_diffusion/conf] \
#     +trainer_configs=latent_diffusion/with_feature_extraction \
...
#     experiment_name=iitcdip_unaug_class_cond_pretraining_klf4 \
#     features_key=klf4 \
#     do_train=False \
#     do_test=False
#     # we use test run just to stop the training right after features are extracted and saved
